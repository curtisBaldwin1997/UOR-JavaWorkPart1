...
public class Room implements Serializable {
	 ...
	 * 
	 * @param rStr
	 *            has xcoords of opposite corners and door as numbers separated by
	 *            spaces
	 */
	public Room(String rStr) {
		StringSplitter m = new StringSplitter(rStr, " "); // Split up co oordinates
		x1 = m.getNthInt(0, 0);
		y1 = m.getNthInt(1, 0);
		x2 = m.getNthInt(2, 5);
		y2 = m.getNthInt(3, 5);
	...
	/**
	 * Defining set location
	 * 
	 * @return
	 */
	public Point locationPerson() {
		Point t = new Point(x2 / 2, y2 / 2); // Setting the location
		return t; // returning point t
	}
	
	public Point locationLight() {
		Point t = new Point(x2 / 3, y2 / 3); // Setting the location
		return t; // returning point t
	}
	 ...
	 * 
	 * @param bi
	 *            interface into which it is shown
	 * @param xa
	 *            start x coord of wall
	 * @param ya
	 *            and y
	 * @param xb
	 *            end x coord of wall
	 ...
	 * 
	 * @param forinside
	 *            being 1 if inside 0 if at door and -1 if outside
	 * @return
	 */
	...
	public boolean isInRoom(Point xy) {
		return (xy.getX() > x1 && xy.getX() < x2 && xy.getY() > y1 && xy.getY() < y2); // seeing if person is inroom or
																						// not
	}
